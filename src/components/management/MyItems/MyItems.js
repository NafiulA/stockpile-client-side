...
const MyItems = () => {
    ...
    useEffect(() => {
        async function getData() {
            let res = await fetch(`https://stockpile.onrender.com/myitem?email=${email}&size=10&page=${page}`, {
                headers: {
    ...
    const handleDelete = id => {
        ...
        if (consent) {
            ...
            fetch(`https://stockpile.onrender.com/deleteinventory/${id}`, {
                method: "DELETE"
            })
                .then(res => res.json())
                .then(data => {
    ...
    useEffect(() => {
        fetch(`https://stockpile.onrender.com/myitemCount?email=${email}`)
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className='min-h-screen'>
            <h3 className='text-4xl font-bold text-slate-800 text-center pt-24 pb-2'>My Inventories</h3>
